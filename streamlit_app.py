...
if uploaded_file is not None:
    ...
    for r in results:
        for box in r.boxes:
            ...
            cls_name, (x1, y1, x2, y2) = model.names[cls], map(int, box.xyxy[0])
    ...
    for person in persons:
        ...
        associated_ppe = [ppe["class"] for ppe in ppe_items if calculate_iou(person_bbox, ppe["bbox"]) > 0.1 or is_nearby(person_bbox, ppe["bbox"])]
        ...
        cv2.rectangle(output_image, (x1, y1 - label_size[1] - 10), (x1 + label_size[0], y1), box_color, -1)
        cv2.putText(output_image, label, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    ...
    for ppe in ppe_items:
        ...
        cv2.putText(output_image, label, (x1, y1 - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
    ...
    compliance_data = pd.DataFrame({"Status": ["Compliant", "Partially Compliant", "Non-Compliant"], "Count": [compliant, partial_compliant, non_compliant]})
    fig = px.pie(compliance_data, values="Count", names="Status", title="PPE Compliance Distribution", color_discrete_sequence=["green", "yellow", "red"])
    st.plotly_chart(fig)

    st.markdown("""**Compliance Legend:**\n- 🟢 Green: All 3 PPE items detected (Hardhat, Mask, Safety Vest)\n- 🟡 Yellow: 1 or 2 PPE items detected\n- 🔴 Red: No PPE detected or violations present""")
    ...
    if len(st.session_state["detection_history"]) > 1:
        ...
        fig_trend = px.line(hist_df, x="timestamp", y=["compliant", "partial", "non_compliant"], title="Compliance Trends Over Time")
